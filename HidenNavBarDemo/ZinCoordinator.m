...
// Constants
static CGFloat const ZinCoordinatorNavigationBarHorizontalHeightDifferential = 20.0f; // This is the difference between the size of the navigation bar when it is in portrait and landscape. This should probably be handled more elegantly.
@interface ZinCoordinator()<UIScrollViewDelegate>
...
- (CGFloat)topViewDefaultY
{
    if (UIDeviceOrientationIsLandscape([[UIDevice currentDevice] orientation])) {
        if ([self.topView isKindOfClass:[UINavigationBar class]]) {
            return _topViewDefaultY - [[UIApplication sharedApplication] statusBarFrame].size.height - ZinCoordinatorNavigationBarHorizontalHeightDifferential;
        } else {
